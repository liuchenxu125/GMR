...
def read_bvh(filename, start=None, end=None, order=None):
    ...
    joint_channels = []  # track channels per joint for mixed-channel BVH files
    ...
    for line in f:
        ...
        if rmatch:
            ...
            joint_channels.append(0)  # placeholder, will be updated by CHANNELS line
        ...
        if chanmatch:
            ...
            # Update the last-added joint's channel count
            if joint_channels:
                joint_channels[-1] = channels
            if order is None:
        ...
        if dmatch:
            ...
            # Use per-joint channel counts for mixed-channel BVH files
            if joint_channels and any(jc != joint_channels[0] for jc in joint_channels):
                # Mixed channels: parse per joint
                offset = 0
                for ji in range(N):
                    nc = joint_channels[ji]
                    if nc == 3:
                        rotations[fi, ji] = data_block[offset:offset+3]
                        offset += 3
                    elif nc == 6:
                        positions[fi, ji] = data_block[offset:offset+3]
                        rotations[fi, ji] = data_block[offset+3:offset+6]
                        offset += 6
                    elif nc == 9:
                        positions[fi, ji] = data_block[offset:offset+3]
                        rotations[fi, ji] = data_block[offset+3:offset+6]
                        ...
                        raise Exception("Unexpected channels per joint: %i for joint %i" % (nc, ji))
            elif channels == 3:
