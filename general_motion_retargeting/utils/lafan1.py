...
def load_bvh_file(bvh_file, format="lafan1"):
    ...
    for frame in range(data.pos.shape[0]):
        ...
        if format == "lafan1":
            ...
            # AMP skeleton: Hips is a child of Root, legs use LeftLeg/LeftShin naming,
            # and toes use LeftToeBase (similar to nokov)
            result["LeftFootMod"] = [result["LeftFoot"][0], result["LeftToeBase"][1]]
            result["RightFootMod"] = [result["RightFoot"][0], result["RightToeBase"][1]]
        elif format == "soma":
            # SOMA skeleton: same bone names as AMP (LeftLeg/LeftShin/LeftToeBase),
            # but ALL joints have 6 DOF (pos+rot channels)
            result["LeftFootMod"] = [result["LeftFoot"][0], result["LeftToeBase"][1]]
            result["RightFootMod"] = [result["RightFoot"][0], result["RightToeBase"][1]]
        else:
