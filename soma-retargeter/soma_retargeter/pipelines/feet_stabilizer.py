...
class FeetStabilizer:
    ...
    def __init__(self, config: str):
        ...
        if self.robot_type == 'unitree_g1':
        ...
        elif self.robot_type == 'casbot_02':
            import pathlib, os
            self.robot_builder = newton.ModelBuilder()
            ...
            search_paths = [
                ...
                pathlib.Path.home() / "amp_lab" / "GMR-master" / "assets" / "casbot_02" / "casbot_02.xml",
            ]
            for p in search_paths:
                if p.exists():
                    casbot_mjcf = p
                    break
            if casbot_mjcf is None:
                raise FileNotFoundError(
                    f"casbot_02 MJCF not found. Searched: {search_paths}\n"
                    f"Set CASBOT_02_MJCF_PATH or GMR_ROOT env var, or run from soma-retargeter/ directory.")
            self.robot_builder.add_mjcf(str(casbot_mjcf))
        else:
